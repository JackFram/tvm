...
class PagedKVCache(Object):  # pylint: disable=too-few-public-methods
    ...
    def sparse_attention_with_fused_qkv(
        self,
        layer_id: int,
        qkv: Tensor,
        num_qo_heads: int,
        ...
        """Compute sparse attention with the given fused q/k/v data and in-cache k/v data
        on the specified layer. Rotary position embeddings are applied to k/v
        within this function.
    ...
    def topk_attention_with_fused_qkv(
        self,
        layer_id: int,
        qkv: Tensor,
        num_qo_heads: int,
        ...
        """Compute attention + qk_product with the given fused q/k/v data and in-cache k/v data
        on the specified layer. Rotary position embeddings are applied to k/v
        within this function.
        ...
        attn_results = bb.emit(
                rx.call_dps_packed(
                    ...
                    out_sinfo=[
                        rx.TensorStructInfo((b * s, num_qo_heads, d), qkv.dtype),
                        rx.TensorStructInfo((max_seq_len, b, num_qo_heads), qkv.dtype),
                    ]
                )
            )
        assert isinstance(attn_results.struct_info, rx.TupleStructInfo)
        assert len(attn_results.struct_info.fields) == 2
        o = Tensor(_expr=bb.emit(rx.TupleGetItem(attn_results, 0))).reshape(b, s, num_qo_heads, d)
        qk_product = Tensor(_expr=bb.emit(rx.TupleGetItem(attn_results, 1)))
        return o, qk_product

    # TODO(Zhihao): finish this stuff (update_tidal, argtopk_func)
    def update_tidal(
        self,
        seq_ids: List[int],
        topk_indices: List[int],
    ) -> None:
        """Update tidal_page_indices with the given seq_ids and topk_indices."""

        _expr=rx.BlockBuilder.current().emit(
                rx.call_dps_packed(
                    "vm.builtin.attention_kv_cache_attention_update_tidal",
                    [
                        self._expr,
                        rx.ShapeExpr(seq_ids),
                        rx.ShapeExpr(topk_indices),
                    ],
                )
            )
    ...
    def argtopk_func(
        ...
        s, b, nh = qk_product._expr.struct_info.shape
        bb = rx.BlockBuilder.current()
        topk_results = bb.emit(rx.op.topk(qk_product, k=token_budget, axis=0, dtype="int32"))
        assert isinstance(topk_results.struct_info, rx.TupleStructInfo)
        assert len(topk_results.struct_info.fields) == 2
        topk_values = Tensor(_expr=bb.emit(rx.TupleGetItem(topk_results, 0)))
        topk_indices = Tensor(_expr=bb.emit(rx.TupleGetItem(topk_results, 1)))
    ...
    def append_mla_kv(self, layer_id: int, kv: Tensor) -> "PagedKVCache":
        ...
        # pylint: disable=protected-access
        b, s, d_qk = kv._expr.struct_info.shape
    ...
    def __init__(  # pylint: disable=too-many-locals
        ...
        mha_functions = (
            [
                ...
                rx.Tuple([rx.StringImm("flashinfer"), rx.ExternFunc("topk_batch_prefill_with_paged_kv_cache_run"), rx.ExternFunc("batch_prefill_with_kv_cache_plan")]),
        ...
        attn_merge_functions = [
            bb.add_func(_merge_state_inplace(num_attention_heads, v_head_dim, dtype, target), "tir_attention_merge_state"),
        ]
        if attn_kind == "mla":
...
class TIRPagedKVCache(PagedKVCache):  # pylint: disable=too-few-public-methods
    ...
    def __init__(  # pylint: disable=too-many-locals
        ...
        if str(target.kind) == "llvm":
            ...
            args.extend(
                [
                    rx.Tuple([rx.StringImm("tir"), bb.add_func(_attention_prefill_ragged_cpu(num_key_value_heads, num_attention_heads, qk_head_dim, dtype, rope_scaling), "tir_attention_prefill_ragged_cpu")]),
            ...
            # fmt: off
            # Todo: adapt to MLA
            args.append(rx.Tuple([rx.StringImm("tir"), bb.add_func(_attention_prefill_ragged(num_key_value_heads, num_attention_heads, qk_head_dim, dtype, rope_scaling, target), "tir_attention_prefill_ragged")]))
            mha_functions = (
                [
...
def _kv_cache_transpose_append(num_key_value_heads, head_dim, dtype):
    ...
    def tir_kv_cache_transpose_append(
        ...
        pages = T.match_buffer(var_pages, (num_pages, 2, num_key_value_heads, global_page_size, head_dim), dtype, elem_offset=pages_elem_offset)
        ...
        for global_pos, h, f in T.grid(ntoken, num_key_value_heads, head_dim):
            if position_map[global_pos] != T.int32(-1):
                with T.block("k_transpose_append"):
                    ...
                    T.writes(pages[position_map[vgpos] // global_page_size, 0, vh, position_map[vgpos] % global_page_size, vf])
                    position: T.int32 = position_map[vgpos]  # type: ignore
                    pages[T.floordiv(position, global_page_size), 0, vh, T.floormod(position, global_page_size), vf] = k_data[vgpos, vh, vf]
                with T.block("v_transpose_append"):
                    vgpos, vh, vf = T.axis.remap("SSS", [global_pos, h, f])
                    T.reads(position_map[vgpos], v_data[vgpos, vh, vf])
                    T.writes(pages[position_map[vgpos] // global_page_size, 1, vh, position_map[vgpos] % global_page_size, vf])
                    position: T.int32 = position_map[vgpos] # type: ignore[name-defined,no-redef]
                    pages[T.floordiv(position, global_page_size), 1, vh, T.floormod(position, global_page_size), vf] = v_data[vgpos, vh, vf]
    # fmt: on
    # pylint: enable=line-too-long
...
def _kv_cache_transpose_append_mla(d_qk: int, dtype):
    ...
    def tir_kv_cache_transpose_append_mla(
        ...
        pages_elem_offset = T.int64()
        position_map_elem_offset = T.int32()
        pages = T.match_buffer(var_pages, (num_pages, global_page_size, d_qk), dtype, elem_offset=pages_elem_offset)
        ...
        for global_pos, f in T.grid(ntoken, d_qk):
            if position_map[global_pos] != T.int32(-1):
                with T.block("k_transpose_append"):
                    ...
                    T.writes(pages[position_map[vgpos] // global_page_size, position_map[vgpos] % global_page_size, vf])
                    position: T.int32 = position_map[vgpos]  # type: ignore
                    pages[T.floordiv(position, global_page_size), T.floormod(position, global_page_size), vf] = kv_data[vgpos, vf]
    # fmt: on
    # pylint: enable=line-too-long
...
def _attention_prefill_cpu(h_kv, h_q, d, dtype, sliding_window: bool, rope_scaling: Dict[str, Any]):
    ...
    def batch_prefill_paged_kv_cpu(
        ...
        pages = T.match_buffer(var_pages, (max_num_pages, 2, h_kv, global_page_size, d), dtype)
            ...
            for b_idx in T.serial(batch_size):
                with T.block("attn"):
                    ...
                    kv_chunk_len[0] = T.if_then_else(
                        ...
                        _get_kv_chunk_len(cur_page_indptr_end - cur_page_indptr_begin, global_page_size, b_idx, length_info, sliding_window),
                        0
                    )
                    ...
                    for q_idx in T.serial(q_indptr[b_idx + 1] - q_indptr[b_idx]):
                        ...
                        for row_idx in T.serial(max_num_pages * global_page_size):
                            if row_idx < kv_chunk_len[0]:
                                ...
                                page_no: T.int32(is_size_var=True) = page_values[cur_page_indptr_begin + (_get_seq_offset(row_idx, b_idx, length_info, sliding_window) // global_page_size)]
                                page_offset: T.int32(is_size_var=True) = _get_seq_offset(row_idx, b_idx, length_info, sliding_window) % global_page_size
...
def _attention_prefill(
    ...
    def batch_prefill_paged_kv(
        ...
        q = T.match_buffer(var_q, (total_len, h_q, d), dtype)
        q_indptr = T.match_buffer(var_q_indptr, (batch_size + 1,), "int32", elem_offset=q_indptr_elem_offset)
        pages = T.match_buffer(var_pages, (max_num_pages, 2, h_kv, global_page_size, d), dtype, elem_offset=pages_elem_offset)
        page_indptr = T.match_buffer(var_page_indptr, (batch_size + 1,), "int32", elem_offset=page_indptr_elem_offset)
        page_values = T.match_buffer(var_page_values, (nnz_pages,), "int32", elem_offset=page_values_elem_offset)
        k_rope_pos_offset = T.match_buffer(var_k_rope_pos_offset, (batch_size,), "int32", elem_offset=k_rope_pos_offset_elem_offset)
        q_rope_position = T.match_buffer(var_q_rope_position, (total_len,), "int32", elem_offset=q_rope_position_elem_offset)
                                        ...
                                        for lz, ly in T.grid(tile_z, tile_y):
                                            with T.block("K_load"):
                                                ...
                                                if cur_L < kv_chunk_len[0]:
                                                    ...
                                                    page_no: T.int32(is_size_var=True) = page_values[cur_page_indptr_begin + T.floordiv(seq_offset, global_page_size)]  # type: ignore
                                                    page_offset: T.int32(is_size_var=True) = T.floormod(seq_offset, global_page_size)  # type: ignore
...
def _attention_decode_cpu(
    ...
    def batch_decode_paged_kv(
        ...
        pages = T.match_buffer(pages_handle, (max_num_pages, 2, H_kv, global_page_size, D), qkv_dtype)
            ...
            with T.block("attn"):
                ...
                for h_qo in T.serial(H_qo):
                    ...
                    for row_idx in T.serial(kv_chunk_len[0]):
                        seq_offset: T.int32(is_size_var=True) = _get_seq_offset(
                            row_idx, b, length_info, sliding_window
                        )
                        page_no: T.int32(is_size_var=True) = page_table_values[
                            cur_page_indptr_begin + (seq_offset // global_page_size)
                        ]
                        page_offset: T.int32(is_size_var=True) = seq_offset % global_page_size
                        ...
                        for d in T.serial(D):
                            K_local[d] = T.if_then_else(
                                ...
                                _rope(
                                    ...
                                    rope_theta,
                                    rope_scale,
                                    (page_no, 0, h_qo // group_size, page_offset, d),
                                    qkv_dtype,
                                    rope_scaling,
                                ),
...
def _attention_decode(
    ...
    def batch_decode_paged_kv(
        ...
        pages = T.match_buffer(
            pages_handle, (max_num_pages, 2, H_kv, global_page_size, D), qkv_dtype, elem_offset=pages_elem_offset
        )
                    ...
                    for tx in T.thread_binding(bdx, thread="threadIdx.x"):
                        for tz in T.thread_binding(bdz, thread="threadIdx.z"):
                            with T.block("attn"):
                                ...
                                kv_chunk_len[0] = T.if_then_else(
                                    cur_page_indptr_begin != cur_page_indptr_end,
                                    _get_kv_chunk_len(cur_page_indptr_end - cur_page_indptr_begin, global_page_size, batch_idx, length_info, sliding_window),
                                    0
                                )
                                    ...
                                    for j in T.serial(tile_size_per_bdx):
                                        with T.block("KV_load"):
                                            ...
                                            if row_g < kv_chunk_len[0]:
                                                ...
                                                page_no: T.int32(is_size_var=True) = page_table_values[cur_page_indptr_begin + T.floordiv(seq_offset, global_page_size)]  # type: ignore
                                                page_offset: T.int32(is_size_var=True) = T.floormod(seq_offset, global_page_size)  # type: ignore
                                                for vec in T.vectorized(VEC_SIZE):
...
def _attention_prefill_ragged(h_kv, h_q, d, dtype, rope_scaling: Dict[str, Any], target: Target):
    ...
    sch = _schedule_prefill_kernel(
        sch, LOAD_VEC, bdx, num_warps, tile_x, tile_y, tile_z, True, False
    )
    return sch.mod["main"].with_attr("tir.is_scheduled", 1)
...
def _attention_prefill_mla(
    ...
    def batch_prefill_paged_kv_mla(
        ...
        q_indptr = T.match_buffer(var_q_indptr, (batch_size + 1,), "int32", elem_offset=q_indptr_elem_offset)
        pages = T.match_buffer(var_pages, (max_num_pages, global_page_size, d_qk), dtype, elem_offset=pages_elem_offset)
        page_indptr = T.match_buffer(var_page_indptr, (batch_size + 1,), "int32", elem_offset=page_indptr_elem_offset)
        page_values = T.match_buffer(var_page_values, (nnz_pages,), "int32", elem_offset=page_values_elem_offset)
...
def _compact_kv_copy(num_heads, head_dim, dtype, target: Target):
    ...
    def compact_kv_copy(
        ...
        pages_elem_offset = T.int64()
        pages = T.match_buffer(
            var_pages, (num_pages, 2, num_heads, global_page_size, head_dim), dtype, elem_offset=pages_elem_offset
        )
                ...
                for bhd_i in T.thread_binding(tx, thread="threadIdx.x"):
                    ...
                    if (bhd_o * tx + bhd_i) < batch_size * num_heads * head_dim:
                        for i in T.serial(copy_length_indptr[b + 1] - copy_length_indptr[b]):
                            ...
                            pages[dst_pos // global_page_size, 0, h, dst_pos % global_page_size, d] = pages[
                                src_pos // global_page_size, 0, h, src_pos % global_page_size, d
                            ]
                            pages[dst_pos // global_page_size, 1, h, dst_pos % global_page_size, d] = pages[
                                src_pos // global_page_size, 1, h, src_pos % global_page_size, d
                            ]
...
def _compact_kv_copy_cpu(num_heads, head_dim, dtype):
    ...
    def compact_kv_copy_cpu(
        ...
        copy_length_indptr_elem_offset = T.int32()
        copy_src_dst_pos_elem_offset = T.int32()
        pages = T.match_buffer(var_pages, (num_pages, 2, num_heads, global_page_size, head_dim), dtype)
        copy_length_indptr = T.match_buffer(
            var_copy_length_indptr,
            (batch_size + 1,),
            "int32",
